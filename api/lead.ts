...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/cabinet\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Cabinet Refacing`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Cabinet Refacing',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Cabinet Alerts <alerts@mursenmaintenance.com>',
